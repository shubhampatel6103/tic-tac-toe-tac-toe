...
(function (CVP) {
    ...
    const TT = new Map();
    function encodeFromState(state) {
        let encoded = 0;
        for (let i = 0; i < 9; i++) {
            const status = state.grids[i].gridStatus;
            let code = CODE_IN_PROGRESS;
            if (status === CVP.GridStatus.DRAW)
                code = CODE_DRAW;
            else if (status === CVP.GridStatus.X_WON)
                code = CODE_X;
            else if (status === CVP.GridStatus.O_WON)
                code = CODE_O;
            encoded |= (code & 0b11) << (i * 2);
        }
        return encoded;
    }
    function bitboardsFromEncoded(encoded) {
        let xBits = 0;
        let oBits = 0;
        for (let i = 0; i < 9; i++) {
            ...
            if (twoBits === CODE_X)
                xBits |= 1 << i;
            else if (twoBits === CODE_O)
                oBits |= 1 << i;
        }
        return { xBits, oBits };
    }
    function countWins(encoded) {
        let xWins = 0;
        let oWins = 0;
        for (let i = 0; i < 9; i++) {
            const twoBits = (encoded >> (i * 2)) & 0b11;
    ...
    function allBigGridsEnded(encoded) {
        for (let i = 0; i < 9; i++) {
            const twoBits = (encoded >> (i * 2)) & 0b11;
            if (twoBits === CODE_IN_PROGRESS)
                return false;
        }
        return true;
    }
    function encodeBigGridStatus() {
        const state = currentStateFromCVP();
        return encodeFromState(state);
    }
    CVP.encodeBigGridStatus = encodeBigGridStatus;
    function evaluateTerminalState() {
        const state = currentStateFromCVP();
        return evaluateTerminalStateFromState(state);
    }
    CVP.evaluateTerminalState = evaluateTerminalState;
    function evaluateTerminalStateFromState(state) {
    ...
    function countOpenTwoInRowPatterns(g, player) {
        const opp = player === 'O' ? 'X' : 'O';
        let cnt = 0;
        for (let i = 0; i < 3; i++) {
            const row = [g[i][0], g[i][1], g[i][2]];
            if (row.filter(v => v === player).length === 2 && row.includes('') && !row.includes(opp))
                cnt++;
            const col = [g[0][i], g[1][i], g[2][i]];
            if (col.filter(v => v === player).length === 2 && col.includes('') && !col.includes(opp))
                cnt++;
        }
        const d1 = [g[0][0], g[1][1], g[2][2]];
        if (d1.filter(v => v === player).length === 2 && d1.includes('') && !d1.includes(opp))
            cnt++;
        const d2 = [g[0][2], g[1][1], g[2][0]];
        if (d2.filter(v => v === player).length === 2 && d2.includes('') && !d2.includes(opp))
            cnt++;
        return cnt;
    }
    function evaluatePosition(state) {
        ...
        for (const mask of WIN_MASKS) {
            ...
            for (let i = 0; i < 9; i++) {
                ...
                if (st === CVP.GridStatus.O_WON)
                    oWon++;
                else if (st === CVP.GridStatus.X_WON)
                    xWon++;
                else if (st === CVP.GridStatus.DRAW)
                    draws++;
                else
                    inprog++;
            }
            if (oWon === 2 && inprog === 1)
                score += EVAL.macroTwoInRow;
            if (xWon === 2 && inprog === 1)
                score -= EVAL.macroTwoInRow;
        }
        const center = 4;
        const corners = [0, 2, 6, 8];
        if (state.grids[center].gridStatus === CVP.GridStatus.O_WON)
            score += EVAL.macroCenterWin;
        else if (state.grids[center].gridStatus === CVP.GridStatus.X_WON)
            score -= EVAL.macroCenterWin;
        for (const idx of corners) {
            if (state.grids[idx].gridStatus === CVP.GridStatus.O_WON)
                score += EVAL.macroCornerWin;
            else if (state.grids[idx].gridStatus === CVP.GridStatus.X_WON)
                score -= EVAL.macroCornerWin;
        }
        for (let i = 0; i < 9; i++) {
            if (state.grids[i].gridStatus !== CVP.GridStatus.IN_PROGRESS)
                continue;
    ...
    function nextValidGrids(state, lastR, lastC) {
        const idx = lastR * 3 + lastC;
        if (state.grids[idx].gridStatus === CVP.GridStatus.IN_PROGRESS)
            return [idx];
        const res = [];
        for (let i = 0; i < 9; i++)
            if (state.grids[i].gridStatus === CVP.GridStatus.IN_PROGRESS)
    ...
    function applyMove(state, g, r, c, mark) {
        state.grids[g].grid[r][c] = mark;
        const res = checkWinLocal(state.grids[g].grid);
        if (res === 'X')
            state.grids[g].gridStatus = CVP.GridStatus.X_WON;
        else if (res === 'O')
            state.grids[g].gridStatus = CVP.GridStatus.O_WON;
        else if (res === 'D')
            state.grids[g].gridStatus = CVP.GridStatus.DRAW;
    ...
    function immediateHeuristicForMove(_stateBefore, stateAfter, g, r, c, mark) {
        let score = 0;
        ...
        if (statusAfter === CVP.GridStatus.O_WON)
            score += mark === 'O' ? 50 : -50;
        else if (statusAfter === CVP.GridStatus.X_WON)
            score += mark === 'X' ? -50 : 50;
        else if (statusAfter === CVP.GridStatus.DRAW)
            score += 0;
    ...
    function stateKey(state, isMax, depth) {
        let s = isMax ? 'M' : 'm';
        s += ':' + depth + ':';
        for (let i = 0; i < 9; i++) {
            const st = state.grids[i].gridStatus;
    ...
    function minimax(state, depth, isMaximizing, alpha, beta, extendBudget) {
        const terminal = evaluateTerminalStateFromState(state);
        if (terminal !== null)
            return terminal;
        if (depth === 0)
            return evaluatePosition(state);
        if (state.validGrids.length === 0)
            return 0;
